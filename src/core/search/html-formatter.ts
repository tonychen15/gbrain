...
function renderCard(row: HtmlResultRow, rank: number): string {
  ...
  const snippet = escapeHtml(r.chunk_text ?? '');
...
:root{
  ...
  --muted:#9aa3b2; --accent:#6ea8fe; --score:#7ee787;
}
...
.page-header{padding:28px 32px 18px;border-bottom:1px solid var(--line)}
...
main{padding:20px 32px 48px;max-width:980px}
...
.badge.stale{background:#3a2a00;color:#ffd479;border:1px solid #5a4200;
...
.snippet{margin:8px 0 10px;padding:10px 14px;border-left:3px solid var(--accent);
  background:#11141a;border-radius:0 6px 6px 0;white-space:pre-wrap;word-break:break-word;
